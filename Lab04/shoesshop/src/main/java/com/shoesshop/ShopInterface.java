...
public class ShopInterface extends JFrame {
    ...
    public ImageIcon getScaledIcon(String path, int width, int height) {
        ...
        ImageIcon imageIcon;
        if (imgURL != null) {
            imageIcon = new ImageIcon(imgURL); // Tải ảnh từ resources thành công
        } else {
            System.err.println("Không tìm thấy ảnh trong resources: " + resourcePath);
            // Trả về một ảnh trống hoặc icon mặc định để giao diện không bị lỗi gạch đỏ
